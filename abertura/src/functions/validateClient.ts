...
export function validateClient() {
  ...
  const freezerIpt = <HTMLInputElement>document.getElementById('freezer-ipt')!;
  ...
  const newClient: Client = {
    ...
    setor: setorIpt.value,
    freezer: freezerIpt.value,
  };
  ...
  if (newClient.nascimento.length !== 8) {
    alert('Data de Nascimento Inválida!\nInforme uma data no formato DDMMYYYY');
  ...
  if (newClient.setor === '0') {
    alert('Selecione o setor!');

    invalidateInput(setorIpt);

    return;
  }

  if (newClient.freezer === '0') {
    alert('Selecione o modelo de freezer!');

    invalidateInput(freezerIpt);
